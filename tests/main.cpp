...
int main()
{
    ...
    tests.push_back(std::string(R"(   '//this is not a comment, it's i/**/n the st/**/ring, // but, on/**/ly for the first part and those are comments since the string is terminated   )"));
    tests.push_back(std::string(R"(   "//this is not a comment, it's /*in the string*/ ", and '//those are not comments as well' since in the single qoutes./* and in the comments */"   )"));
    //
    begin_test();
    return 0;
}
